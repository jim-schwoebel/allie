...
def seed_files(filename, cur_dir, train_dir, clean_data, augment_data, filetype):
    ...
    if clean_data==False and augment_data==False:
...
class SimplisticTest(unittest.TestCase):
    ...
    os.chdir(train_dir)
    # only for the first one, remove if directory exists.
    try:
        os.mkdir('one')
    except:
        shutil.rmtree('one')
    ...
    def test_compression(self, model_compress=model_compress, loadmodel_dir=loadmodel_dir):
        b=False
    ...
    def test_production(self, production=production, production_dir=production_dir):
        b=False
        if production == True:
